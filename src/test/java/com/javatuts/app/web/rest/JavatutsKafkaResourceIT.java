...
import com.javatuts.app.config.KafkaProperties;
...
class JavatutsKafkaResourceIT {
    ...
    void setup() {
        ...
        JavatutsKafkaResource kafkaResource = new JavatutsKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/javatuts-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/javatuts-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
